...
void outputToCMDforS_T_R_SPCC_Combination(SensorlogType1& sensorData, int month) {
	...
	std::cout << "\nPearson Correlation Coefficient for " + monthList[month - 1] << ":\n";
...
void outputWholeDATAtoCMD(SensorlogType1& sensorData, int year) {
	try {
		...
		std::cout << "Month,Average Wind Speed(stdev), Average Ambient Temperature(stdev), Solar Radiation\n\n";
		...
		for (int i = 1; i <= monthNum; ++i) {
			...
			if (SRsum == 0.0 || SPEEDavg == 0.0 || TEMPavg == 0.0) {
				...
				std::cout << monthList[i - 1] << ": " << SPEEDavg << "(" << SPEEDstddvi << ")," << TEMPavg << "(" << TEMPstddvi << ")," << (SRsum) << "\n";
			}
		}
		std::cout << "\n";
	}
...
void outputToCSV(std::ofstream& outFile, SensorlogType1& sensorData, const int year) {
	outFile << "Year" << "," << year << "\n";
	outFile << "Month,Avger Wind Speed(Stdev),Average Temperature(Stdev),Solar Radiation\n";
	...
	for (int i = 1; i <= monthNum; ++i) {
		...
		if (SRsum == 0.0 || SPEEDavg == 0.0 || TEMPavg == 0.0) {
			...
			outFile << monthList[i - 1] << "," << SPEEDavg << "(" << SPEEDstddvi << ")" << "," << TEMPavg << "(" << TEMPstddvi << ")" << "," << (SRsum) << "\n";
		}
	}
	outFile.close();
